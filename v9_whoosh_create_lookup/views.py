...
schema = Schema(term=NGRAM(minsize=2, maxsize=20,stored=True), product=TEXT, url=STORED(), description=STORED(), preview=STORED())
...
class SearchIndex():
    ...
    def add_terms(self, keywords, product, url, description, explanation=None):
        print "adding_terms: keywords= "
        print keywords
        ...
        self.collector.append([terms, product, url, description, explanation])
    ...
    def create_whoosh(self):
        ...
        if create_index_flag:
            ...
            for t in self.collector:
                ...
                print "index: Adding product= %s,"%t[1]
                #writer.add_document(term=u"%s"%t[0], url=u"%s"%t[1], description=u"%s"%t[2])
                writer.add_document(term=u"%s"%t[0], product=u"%s"%t[1], url=u"%s"%t[2], description=u"%s"%t[3])
...
###########################################
    ...
    if action == 'get_sps_tags' :
        ...
        for found in searchindex.lookup(temp):
            print "type of found: ", type(found) # l
            if found['product'] != None:
                print "product found:", found['product']
                reqproduct = found['product']
        ...
        if reqproduct == "":
            ...
            if versions != None:
                ...
                for v in versions:
                    ...
                    searchindex.add_terms( keywords = v['VersionName'], product = unicode(keyid), url=None,  description=None, explanation=None);
